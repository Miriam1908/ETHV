...
console.log('[LikeTalent] MINTER:', MINTER_KEY   ? 'OK' : 'FALTA - no se podran emitir certificados');
...
const TOOLS = [
  {
    ...
    function: {
      name: 'analyze_cv',
      description: 'Descarga y analiza un CV desde una URL (Google Drive, Dropbox, link directo a PDF/DOCX).',
      parameters: {
        type: 'object',
        properties: { url: { type: 'string', description: 'URL del archivo CV (PDF o DOCX)' } },
        required: ['url']
      }
    }
  },
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Genera una carta de presentacion profesional basada en el CV del usuario.',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Inicia un quiz de validacion de un skill tecnico especifico.',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Emite un certificado en blockchain. Solo cuando el usuario aprobo el quiz y dio su wallet.',
      parameters: {
        type: 'object',
        properties: {
...
async function evaluateQuiz(skill, questions, answers) {
  const prompt = 'Eres un evaluador tecnico experto. El usuario respondio un quiz de "' + skill + '".\n\nPreguntas y respuestas:\n' +
    questions.map(function(q, i) {
      return (i + 1) + '. ' + q.question + '\n   Respuesta: ' + (answers[i] || '(sin respuesta)');
    }).join('\n') +
    '\n\nDevuelve EXACTAMENTE este JSON (sin markdown):\n{"score":85,"level":"Mid","passed":true,"evaluation":"Evaluacion breve en espanol."}';
  ...
  if (!match) throw new Error('Respuesta del evaluador invalida');
...
async function executeTool(toolName, args, session) {
  ...
  if (toolName === 'generate_cover_letter') {
    ...
    const target  = (args && args.job_title) ? ' para el puesto de ' + args.job_title : '';
    const company = (args && args.company)   ? ' en ' + args.company : '';
    const prompt  = 'Genera una carta de presentacion profesional en espanol' + target + company + ' para ' + (cv.name || 'el candidato') + ', ' + (cv.current_position || 'profesional') + ' con skills en ' + (cv.skills || []).slice(0, 5).join(', ') + '. Formal, 3 parrafos, lista para enviar. Solo devuelve la carta.';
  ...
  if (toolName === 'start_skill_quiz') {
    await wakeBackend();
    const result = await callBackend('/api/generate-quiz', { skill: args.skill, level: (args && args.level) || 'mid', lang: 'es' });
  ...
  if (toolName === 'mint_certificate') {
    if (!ethers.isAddress(args.wallet_address)) return JSON.stringify({ error: 'Wallet address invalida.' });
  ...
  if (toolName === 'get_user_profile') {
    if (!session.cvData) return JSON.stringify({ error: 'No hay CV analizado aun.' });
...
async function send(isChannel, roomId, chatId, msg) {
  ...
  else await agent.sendConnectionMessage(chatId || roomId, msg);
}
...
app.post('/webhook', async function(req, res) {
  ...
  try {
    ...
    // 2. Quiz activo: colectar respuesta
    if (session.quizState && !text.startsWith('/')) {
      ...
      if (quiz.current < quiz.questions.length) {
        ...
        let result;
        try { result = await evaluateQuiz(quiz.skill, quiz.questions, quiz.answers); }
        catch(e) { await send(isChannel, roomId, chatId, 'Error al evaluar. Intenta de nuevo.'); return; }
        ...
        if (passed) {
          ...
          msg += '\n\nAprobaste! Para emitir tu certificado en blockchain, envia tu wallet address (0x...).';
        } else {
          msg += '\n\nNecesitas ' + PASS_SCORE + '/100 para aprobar. Intenta de nuevo con /skills ' + quiz.skill;
        }
        await send(isChannel, roomId, chatId, msg);
      }
      return;
    ...
    // 3. Certificado pendiente + wallet recibida
    ...
    // Si el agente inicio un quiz, enviar primera pregunta
...
app.get('/health', function(req, res) {
  res.json({ status: 'ok', version: 'agent-v3-clean', sessions: sessions.size, minter: !!MINTER_KEY });
});
